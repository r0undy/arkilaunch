...
- Root `vercel.json` (new) builds `@arkilaunch/shared` before `@arkilaunch/web` and points `outputDirectory` at `apps/web/dist` — Vercel's Root Directory must be the repo root, not `apps/web`, since `@arkilaunch/web` depends on `@arkilaunch/shared` via `workspace:*`. `apps/web/vercel.json` is kept as well: whichever directory the Vercel project is actually rooted at, one of the two is read and both carry the same SPA rewrite. **Correction (2026-09-15):** the root file described here was never committed -- only `apps/web/vercel.json` existed, and with the project rooted at the repo root it was never read, so no rewrite applied and every deep link (`/login`, `/app`, `/app/ocr`) returned Vercel's 404. Only `/` worked, and only until a refresh. Root `vercel.json` added 2026-09-15.
- `apps/web/.env.example` (new): documents `VITE_API_BASE_URL` and the new `VITE_PUBLIC_SITE_URL`.
- `apps/web/scripts/prerender.mjs`: domain is now `VITE_PUBLIC_SITE_URL`-driven instead of hardcoded `https://almara.example`; the six equipment-detail routes are fetched live from `GET /catalog/equipment` at build time instead of the hardcoded `eq-1`..`eq-6` fixture list (skipped, not faked, if the fetch fails). `robots.txt`/`sitemap.xml` point at the deployment origin (`https://arkilaunch-web-24lk.vercel.app` as of 2026-09-15).
...
- **Per-deployment Vercel preview URLs are not supported**: the API's CORS allowlist (`WEB_ORIGIN`) is a single origin, so only the one origin named by `web_origin` in `infra/terraform/environments/dev/terraform.tfvars` works against the deployed dev API. That was `arkilaunch-dev.vercel.app`, a domain that was never actually created -- so no browser origin matched and every login failed CORS, which the login form reports as "incorrect email or password". Corrected 2026-09-15 to the real deployment origin. Widening this to a preview-URL pattern is a backend/security decision, out of scope here.
