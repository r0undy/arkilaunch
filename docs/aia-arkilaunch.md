...
>
> **Status note (`cr-arkilaunch-ocr-extraction-enablement.md`, 2026-09-16):** the "not exercised" position above is now **half false**. Real extraction has been run against `di-arkilaunch-dev`: `prebuilt-layout` with `queryFields` returned a SEC number and TIN at 0.995 confidence from a synthetic document containing no real personal data, which also settles that `queryFields` is available on the F0 tier. **AIA-R1/R3 are therefore exercised for the KYC path** and AIA-R5 is not, since it concerns handwriting on the EDTR path. That path cannot run at all: `arkilaunch-edtr-neural-v1` returns 404 on the live resource, no custom model of any kind exists on it, and training one needs a labeled corpus that does not exist yet. `ENABLE_OCR_KYC=true` is set in one developer's local environment for verification only. **No deployed environment changed, both Terraform flags remain `false`, and AIA-R7 stays Open (escalated)** with the §4 launch pause fully in force: the resource is in Singapore, so a PH-origin document still crosses a border and running it locally does not make that lawful.
