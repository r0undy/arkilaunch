...
2. **SCRUTINY** ([scrutiny-arkilaunch.md](scrutiny-arkilaunch.md)); verified claims and the carried-gap register (`G-1`..`G-10`) that every downstream doc resolves against.
3. **BRD** ([brd-arkilaunch.md](brd-arkilaunch.md)); the business case, `BRD-M#` metrics, `BRD-V#` impact variables.
4. **PRD** ([prd-arkilaunch.md](prd-arkilaunch.md)); features PRD-F1..F8, user stories, flows, §7 AI spec, §9 rollback, §NFR.
5. **SDD** ([sdd-arkilaunch.md](sdd-arkilaunch.md)); architecture, 35-table schema, APIs, §5 security/RLS, §8 AI architecture.
6. **RFCs**; [rfc-001 tenancy-rls-auth](rfc-arkilaunch-tenancy-rls-auth.md), [rfc-002 ocr-edtr-reconciliation](rfc-arkilaunch-ocr-edtr-reconciliation.md), [rfc-003 quotation-pricing-engine](rfc-arkilaunch-quotation-pricing-engine.md).
7. **DSD** ([dsd-arkilaunch.md](dsd-arkilaunch.md)); the Yardboard design system, tokens, components, a11y.
8. **QAD** ([qad-arkilaunch.md](qad-arkilaunch.md)); test matrix and release criteria.
9. **CLR** ([clr-arkilaunch.md](clr-arkilaunch.md)); PH data-privacy register, KYC, scraping posture.
10. **AIA** ([aia-arkilaunch.md](aia-arkilaunch.md)); AI assurance for the Azure DI OCR component.
...
**Definition of Done (build / release):** ArkiLaunch's own Production Readiness Gate must pass before shipping: the checklist in [docs/index.md](index.md) §4 Health Check, backed by [pitch-arkilaunch.md](pitch-arkilaunch.md) §5 (pre-demo gate) and [wrap-arkilaunch.md](wrap-arkilaunch.md) §5 (final check). The generic FMD-engine gate this pointed at (`fmd/AGENTS.md`) is an external tool this repo does not vendor; the three ArkiLaunch-specific checklists above are the ones that actually gate a release here.
