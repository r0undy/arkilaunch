...
| Project-site geodata | PRD-F5 / `project_sites` (`latitude`, `longitude`) | Poll weather per site; risk advisories; liability evidence | Precise site coordinates (lat/long) | Tenant / customer project sites (site, not person) | **Open-Meteo (FREE tier as of 2026-08-20, `cr-arkilaunch-open-meteo-free-tier.md`)** receives lat/long | **Yes: coordinates sent to Open-Meteo (EU/Germany-hosted).** Non-personal commodity weather query; note in transfer review. **New (same CR): the free tier's data is CC BY 4.0 licensed -- attribution is rendered in the UI (discharged) -- and its terms of use restrict the API to non-commercial use, which ArkiLaunch does not strictly satisfy; this is an accepted, open exposure under SCRUTINY G-4, not a resolved one.** | Advisory life + incident-evidence retention | Legitimate interest (safety evidence) / contract |
