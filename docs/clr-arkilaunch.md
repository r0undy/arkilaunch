...
| EDTR capture & OCR | PRD-F3 / `edtr`, `edtr_line_items`, `edtr_reconciliations` (`raw_file_uri` -> Supabase Storage, `ocr_payload`) | Back-end the paper: extract active/idle hours; reconcile before billing | Handwritten EDTR images (may carry operator names / handwriting), active/idle hours, equipment refs | Equipment operators, timekeepers | **Azure AI Document Intelligence**; Supabase Storage | **Yes: images sent to Azure DI. Region residency UNVERIFIED (flag E1).** | Image + payload retention limited to billing-evidence need; audit trail immutable | Contract / legitimate interest (billing integrity) |
...
| Product analytics / telemetry | PRD §5.6 / PostHog (PH/EU residency) or first-party `events` table on Supabase; **sink TBD** | Wire BRD-M# metrics | Event names + identifiers only; **no PII in property values, no raw SEC/TIN, no card/account numbers, no ID images** (PRD §5.6 naming rule) | Users | PostHog (if chosen; PH/EU-residency region) or none (first-party) | Depends on sink choice; keep PH/EU residency | Rolling analytics window (set with sink choice) | Legitimate interest / consent (confirm with sink decision) |
| Application logs | SDD §7 / structured logs (Winston / Nest Logger) | Ops + debugging | Structured app logs; identifiers only, no PII values, no secrets (password hash / TOTP never logged) | Users (indirect) | Log sink (OPS, TBD) | Keep in-region | 30 to 90 days (SDD §7) | Legitimate interest (operations) |
...
| Analytics / telemetry | **Yes** | PostHog (PH/EU residency) or a first-party `events` table; identifiers only, no PII in values (PRD §5.6). |
...
| **Breach notification** | Authority ≤72h; subjects without undue delay if high risk | Without unreasonable delay; CA AG if >500 residents | **NPC and affected subjects within 72h of knowledge** if the breach involves sensitive PII and a real risk of serious harm. Runbook lives in OPS. |
...
| Breach response runbook exists with the tightest applicable timeline | Not yet | To live in [ops-arkilaunch.md](ops-arkilaunch.md); SDD §5 security controls | **Yes.** Must encode the NPC + subjects 72h line. |
...
|----|------|-----------------|------------------|--------|
| **E1** | **Cross-border transfer of KYC/ID + EDTR images to Azure AI Document Intelligence.** Verify the SE-Asia (Singapore) region and data-residency for PH document images; confirm the RA 10173 Sec. 21 accountability posture and comparable-protection guarantee; confirm Microsoft does not train on customer data and its retention terms. | Counsel + DPO | **Yes** | SDD §4, §8.1; PRD §8 (G-5) |
...
**Launch gate.** This CLR MUST clear (E1 through E6, E8, E10, E11 resolved or formally accepted by counsel) before GTM / public launch, **alongside the AIA** for the AI component. The Production Readiness Gate (fmd/AGENTS.md) and [gtm-arkilaunch.md](gtm-arkilaunch.md) both treat compliance as a hard launch gate.
